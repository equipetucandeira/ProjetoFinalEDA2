...
int main(){
    ...
    //chamar AVL ou LLRB. Para executar os dois, seria necesseário abrir o arquivo novamente.
...
void carregaArquivoArvAVL(arvAVL *raizAVL, FILE *arquivo){
    ...
    while(fgets(aux, sizeof(aux), arquivo)){
        ...
        insere_arvAVL(raizAVL, data.codigo, data.nome, data.idade, data.empresa, data.departamento, data.salario);
    }

    gettimeofday(&depois, NULL);

    deltaT = (depois.tv_sec - antes.tv_sec) + (depois.tv_usec - antes.tv_usec) / 1000000.0;
...
void carregaArquivoArvLLRB(arvLLRB *raizLLRB, FILE *arquivo){
    ...
    while(fgets(aux, sizeof(aux), arquivo)){
        ...
        data = retornaData(arquivo);
        insere_arvoreLLRB(raizLLRB, data.codigo, data.nome, data.idade, data.empresa, data.departamento, data.salario);
    }
...
int ordenaArquivo(FILE *arquivo){
    ...
    for (int j = 0; j < size; j++) {
        fprintf(sortedFile, "%d;%s;%d;%s;%s;%.2f\n",
                v[j].codigo, v[j].nome, v[j].idade, v[j].empresa, v[j].departamento, v[j].salario);
    }
